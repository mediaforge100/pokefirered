...
#define BATTLE_TYPE_POKEPVP            (1 << 20) // POKEPVP (ADR-061): server-authoritative PvP. Both battler slots get PlayerBufferRunCommand as their renderer; battle_controller_pokepvp.c populates gBattleBufferA from the EWRAM mailbox instead of from this ROM's own local battle engine.
